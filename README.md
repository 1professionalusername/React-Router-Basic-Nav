...
* inside the `REACT DEV TOOLS` expand `<BrowserRouter>` and highlight `<Router.Provider>`. Notice that here is a `"history"` object on props and a `"match"` object on its state. These two objects are how all of our Router is going to work. 
